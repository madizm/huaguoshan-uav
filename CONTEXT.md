...
A display object ingested from an external approval or reporting source that represents a unit's planned or approved flight activity in a specified time window. It retains source snapshots such as an aircraft name but does not link to a platform-managed Aircraft Asset.
由外部审批或报备来源接入的平台展示对象，表示某个单位在指定时间窗口内计划或获准开展的飞行活动。它保留来源中的无人机名称等快照，但不关联平台管理的 Aircraft Asset。
...
**Equipment Asset（设施设备资产）**:
An individually registered physical device or logical equipment system that has an identity, location, lifecycle, and connectivity/availability state. The current categories are 6G base stations, counter-UAS devices, video surveillance, UAVs, unmanned vehicles, vehicle-mounted surveillance, and sensors.
具有独立身份、位置、生命周期以及连接/可用状态的物理设备或逻辑设备系统；当前类别包括 6G 基站、反无设备、视频监控、无人机、无人车、车载监控和传感设备。
_Avoid_: Raw Observation, Response Capability, Emergency Resource, 原始观测、处置能力、应急资源

**Equipment Connectivity Status（设备连接状态）**:
The latest known connection state of an Equipment Asset, such as online, offline, degraded, or unknown; it is distinct from whether the asset is available for dispatch.
设施设备资产最近一次已知的连接状态，如在线、离线、异常或未知；它不同于设备是否可被调度。
_Avoid_: Availability Status, Dispatch Status, 可用状态、调度状态

**Dispatchable Equipment（可调度设备）**:
An Equipment Asset that is also registered as an Emergency Resource and may be assigned to a response activity. In the current scope, UAVs, unmanned vehicles, vehicle-mounted surveillance, and video surveillance are dispatchable; counter-UAS devices are display-and-recommend only and are not directly controlled by the platform.
同时登记为应急资源、可以分配给处置活动的设施设备资产。当前范围内，无人机、无人车、车载监控和视频监控可调度；反无设备仅用于展示和推荐，平台不直接控制。
_Avoid_: All Equipment, Direct Device Control, 所有设备、设备直接控制

**Vehicle-Mounted Surveillance（车载监控）**:
A logical surveillance equipment system whose own record stores monitoring capabilities and current state but does not create a separately managed vehicle asset in the current scope.
一种逻辑监控设备系统；当前范围内只管理其监控能力和自身状态，不单独建立或管理载体车辆资产。
_Avoid_: Unmanned Vehicle, Vehicle Fleet, 无人车、车辆车队

**Equipment Raw Observation（设备原始观测）**:
An immutable or append-only simulated or externally supplied observation produced by an Equipment Asset, retaining observation time, source asset, position, height in AMSL, raw payload, confidence, and processing status.
由设施设备资产产生的模拟或外部原始观测，采用不可变或追加写入方式保存，并保留观测时间、来源设备、位置、AMSL 高度、原始载荷、置信度和处理状态。
_Avoid_: Equipment Status, Fused Assessment, 设备状态、融合判断

**Equipment Height Convention（设备高度约定）**:
All equipment locations, current positions, observation heights, and coverage vertical limits use AMSL as the database height datum. The datum must be stored explicitly rather than inferred from a column name or source system.
所有设备位置、实时位置、观测高度和覆盖范围垂直上下限在数据库中统一采用 AMSL；必须显式保存高度基准，不能依靠字段名或来源系统推断。
_Avoid_: Unqualified Height, Relative Height, 未注明高度、相对高度

**System User（系统使用人员）**:
The primary business role that uses the platform to view Equipment Assets, current states, Raw Observations, capability coverage, Emergency Resources, and decision-support recommendations.
使用平台查看设施设备资产、当前状态、原始观测、能力覆盖、应急资源和辅助决策推荐结果的主要业务角色。
_Avoid_: Asset Administrator, Duty Operator, Analyst, 设备管理员、值班人员、研判人员

**System Operations Personnel（系统运维人员）**:
The business role responsible for maintaining Equipment Asset records, source mappings, simulated data, migrations, API configuration, and operational data quality.
负责维护设施设备资产、来源映射、模拟数据、数据迁移、API 配置和运行数据质量的业务角色。
_Avoid_: Separate Device Administrator, Data Administrator, Security Administrator, 独立设备管理员、数据管理员、安全管理员
