...
- 禁飞/管控区入侵和林火疑似预警必须进入同一个 **MVP 事件队列**，不能形成两套列表、生命周期和处置流程。
...
| --- | --- | --- | --- |
| `airspace_intrusion` | 目标航迹进入空域约束并满足入侵确认窗口 | 目标、航迹、约束、命中网格、高度基准、判定结果 | 持续跟踪、人工核查、通知责任单位、模拟联动 |
...
│ 320–360 px   │ 目标/约束/证据/影响区/能力覆盖 │ 400–460 px        │
│              │                               │ 概览/证据/方案/历史│
├──────────────┴───────────────────────────────┴───────────────────┤
...
- 风险颜色只用于事件和影响范围，不污染资产、目标和约束自身颜色。
- 候选事件使用虚线/低透明度；正式事件使用实线。
- 高度值始终同时显示数值、单位和 Height Datum。
...
| `event_type text` | `airspace_intrusion` / `forest_fire_suspected` |
...
| `min_height_m/max_height_m numeric` | 可空的垂直范围 |
| `height_datum text` | `AMSL` / `AGL`，有高度时必填 |
...
- `event_response.intrusion_event_detail`
  - `event_id`
  - `target_id`
  - `target_track_id`
  - `constraint_kind`
  - `constraint_id`
  - `intrusion_started_at`
...
- `event_id`。
- `grid_code`。
- `relation_type`：`impact | evidence | constraint | capability`。
...
- 空域约束复用现有 `airspace.no_fly_zone`、`airspace.temp_control_zone` 及 `api` 视图。
...
- 禁飞/管控区编辑、航线规划仍属于配置/规划模式，不放进事件处置操作流。

## 11. 实施顺序

### Phase 1：事件底座与演示闭环

1. 新建 `event_response` schema、事件/历史/证据/责任/方案/关闭表。
2. 实现列表、详情、认领、状态迁移、选择方案、记录结果和关闭 RPC。
3. 提供两类正式事件的可重复执行演示种子数据。
4. 实现事件队列、详情、地图定位和事件历史。
5. 先支持人工驱动完整闭环，验证领域模型和交互。

### Phase 2：入侵自动形成事件

1. 接入目标航迹与候选事件记录。
2. 实现网格粗筛、PostGIS 精确判定和确认窗口。
3. 实现入侵风险分级和处置能力覆盖匹配。
4. 自动生成可解释处置方案候选集。
