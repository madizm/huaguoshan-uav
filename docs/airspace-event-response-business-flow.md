...
防御圈和评分配置见 [`defense-ring-risk-model-v1.md`](./defense-ring-risk-model-v1.md)。第一阶段先定数据库结构及默认值；下图描述配置落地后的事件流程。
...
- 防御圈的中心、半径、优先级及保护对象。
- 防御圈生效时间。
- 中心到目标位置的米制地理距离是否不大于半径；不按高度筛选。
- 连续观测数量、持续时间和观测置信度。

网格命中只用于粗筛，不能直接等同于防御圈命中。重叠时仅输出一个圈；缺位置或观测无效时输出数据不足，不当作圈外。缺高度不影响防御圈判定。
