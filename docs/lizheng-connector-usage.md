...
- 历正明确返回有效 `deleted_time` 时，立即写入 `offline_remove`，会话和航迹关闭为 `closed`。
- 目标仅从 HTTP 在线快照中缺失时，不合成原始观测；数据库按侦测来源配置的
  `lost_timeout_seconds` 宽限，超时后将会话和航迹标记为 `lost`。
- 目标在宽限期内重新出现时继续关联原会话和原航迹。
...
**说明**:
- 没有无人机消息时连接应保持空闲，不应每 30 秒重连。
- 连接器使用 WebSocket ping/pong 检测失效连接，并在真正断开后自动恢复订阅。
- 持续重连时应检查 VPN、设备服务以及日志中的订阅错误。

### 原始数据留存

历正接口返回的无人机字段会原样写入 `equipment.raw_observation.raw_payload`；标准化结果另存于
`raw_payload._normalized`。`210`、`365` 等厂商无效哨兵值只在标准化字段中转换为空，不会改写
原始载荷，便于后续与历正确认设备输出。
