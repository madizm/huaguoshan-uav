/*
 * Shared Cesium airspace grid renderer.
 *
 * Grid-system adapters provide encoding, bounds, defaults, and labels; this
 * module owns drawing, selection, labels, throttling, and status events.
 */
(function(root, factory) {
    ...
    if (typeof module === "object" && module.exports) {
        ...
        root.AirspaceGridRenderer = factory();
    }
}(typeof self !== "undefined" ? self : this, function() {
    "use strict";

    function createGridModule(gridSystem) {
        var GridCode = gridSystem.GridCode;
        var GridBounds = gridSystem.GridBounds;
        var moduleName = gridSystem.moduleName;
        var displayName = gridSystem.displayName;
        var codeName = gridSystem.codeName;
        var selectedEntityName = gridSystem.selectedEntityName;
        var selectionMessagePrefix = gridSystem.selectionMessagePrefix;
        var missingDependencyMessage = gridSystem.missingDependencyMessage;
        var minLevel = gridSystem.minLevel || 1;
        var maxLevel = gridSystem.maxLevel;
        var fallbackAutoLevel = gridSystem.fallbackAutoLevel;

    var DEFAULTS = gridSystem.defaults;


    var LEVEL_BY_HEIGHT = gridSystem.levelByHeight;
    ...
    function getCesium() {
        ...
        if (typeof Cesium === "undefined") {
            throw new Error("Cesium runtime is required for " + moduleName);
        }
    ...
    function estimateMetersPerPixel(viewer, rectangle) {
        ...
        widthMeters = GridBounds.distanceMeters(rectangle.west, centerLat, rectangle.east, centerLat);
    ...
    function chooseAutoLevel(viewer, rectangle, options, previousLevel) {
        ...
        if (metersPerPixel) {
            for (level = 1; level <= options.maxAutoLevel; level++) {
                bounds = GridBounds.getCellBounds(centerLon, centerLat, level);
        ...
        if (previousLevel && Math.abs(bestLevel - previousLevel) === 1) {
            bounds = GridBounds.getCellBounds(centerLon, centerLat, previousLevel);
    ...
    function expandRectangle(rectangle, level, options) {
        var centerLon = (rectangle.west + rectangle.east) / 2;
        var centerLat = clamp((rectangle.south + rectangle.north) / 2, options.minLatitude, options.maxLatitude);
        var bounds = GridBounds.getCellBounds(centerLon, centerLat, level);
    ...
    function enumerateCells(rectangle, level, maxCells, options) {
        ...
        while (lat < padded.north && guardRows < maxCells + 1000) {
            rowBounds = GridBounds.getCellBounds(clamp(padded.west + epsilon, -180, 180), lat, level);
            lon = clamp(padded.west + epsilon, -180, 180);
            guardCols = 0;

            while (lon < padded.east && guardCols < maxCells + 1000) {
                bounds = GridBounds.getCellBounds(lon, lat, level);
                key = formatCodeKey(bounds);

                if (!seen[key]) {
                    code2D = GridCode.encode2D(bounds.centerLon, bounds.centerLat, level);
                    seen[key] = true;
                    cells.push({
                        level: level,
                        code2D: code2D,
    ...
    function createRenderer(viewer, userOptions) {
        ...
        function getHeightModel(level, cellCount) {
            ...
            layers = GridBounds.getStackedHeightBounds(anchorHeight, level, stack.effective);
            if (!layers.length) {
                layers = [GridBounds.getHeightBounds(anchorHeight, level)];
            }
            ...
            currentLayer = GridBounds.getHeightBounds(currentHeight, level);
        ...
        function refreshNow() {
            ...
            if (!options.enabled) {
                clearCollection(collections.lines);
                clearCollection(collections.labels);
                emitStatus(displayName + "未开启", {enabled: false, cells: 0, level: options.level});
                return;
            }
            ...
            message = displayName + " L" + renderable.level + "：" + renderable.cells.length + " 个可视单元";
            ...
            if (options.verticalEnabled) {
                message += "，" + codeName + " 高度层 " + heightModel.unitHeightMeters.toFixed(2) + "m，堆叠 " + heightModel.stackCount + " 层";
            }
        ...
        function handleClick(movement) {
            ...
            selection = {
                ...
                lat: lat,
                code2D: GridCode.encode2D(bounds.centerLon, bounds.centerLat, level),
                code3D: GridCode.encode3D(bounds.centerLon, bounds.centerLat, currentHeight, level),
                bounds: bounds,
            ...
            emitStatus(selectionMessagePrefix + selection.code2D, {
    ...
    return {
        createGridModule: createGridModule
    };
}));
