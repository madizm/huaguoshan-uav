/*
 * GGER airspace grid visualization for Cesium.
 *
 * Renders the current camera view as 2D / 3D GGER / GeoSOT grid cells and
 * exposes click-to-query GGER metadata. Depends on GGERGridCode,
 * GGERGridBounds and a browser Cesium runtime.
 */
(function(root, factory) {
    "use strict";

    if (typeof module === "object" && module.exports) {
        module.exports = factory(require("./GGERGridCode"), require("./GGERGridBounds"));
    } else {
        root.GGERAirspaceGrid = factory(root.GGERGridCode, root.GGERGridBounds);
    }
}(typeof self !== "undefined" ? self : this, function(GGERGridCode, GGERGridBounds) {
    "use strict";
    ...
    function getCesium() {
        ...
        if (typeof Cesium === "undefined") {
            throw new Error("Cesium runtime is required for GGERAirspaceGrid");
        }
    ...
    function estimateMetersPerPixel(viewer, rectangle) {
        ...
        widthMeters = GGERGridBounds.distanceMeters(rectangle.west, centerLat, rectangle.east, centerLat);
    ...
    function chooseAutoLevel(viewer, rectangle, options, previousLevel) {
        ...
        if (metersPerPixel) {
            for (level = 1; level <= options.maxAutoLevel; level++) {
                bounds = GGERGridBounds.getCellBounds(centerLon, centerLat, level);
        ...
        if (previousLevel && Math.abs(bestLevel - previousLevel) === 1) {
            bounds = GGERGridBounds.getCellBounds(centerLon, centerLat, previousLevel);
    ...
    function expandRectangle(rectangle, level, options) {
        var centerLon = (rectangle.west + rectangle.east) / 2;
        var centerLat = clamp((rectangle.south + rectangle.north) / 2, options.minLatitude, options.maxLatitude);
        var bounds = GGERGridBounds.getCellBounds(centerLon, centerLat, level);
    ...
    function enumerateCells(rectangle, level, maxCells, options) {
        ...
        while (lat < padded.north && guardRows < maxCells + 1000) {
            rowBounds = GGERGridBounds.getCellBounds(clamp(padded.west + epsilon, -180, 180), lat, level);
            lon = clamp(padded.west + epsilon, -180, 180);
            guardCols = 0;

            while (lon < padded.east && guardCols < maxCells + 1000) {
                bounds = GGERGridBounds.getCellBounds(lon, lat, level);
                key = formatCodeKey(bounds);

                if (!seen[key]) {
                    code2D = GGERGridCode.encode2D(bounds.centerLon, bounds.centerLat, level);
                    seen[key] = true;
                    cells.push({
                        level: level,
                        code2D: code2D,
    ...
    function createRenderer(viewer, userOptions) {
        ...
        function getHeightModel(level, cellCount) {
            ...
            layers = GGERGridBounds.getStackedHeightBounds(anchorHeight, level, stack.effective);
            if (!layers.length) {
                layers = [GGERGridBounds.getHeightBounds(anchorHeight, level)];
            }
            ...
            currentLayer = GGERGridBounds.getHeightBounds(currentHeight, level);
        ...
        function refreshNow() {
            ...
            if (!options.enabled) {
                clearCollection(collections.lines);
                clearCollection(collections.labels);
                emitStatus("GGER 网格未开启", {enabled: false, cells: 0, level: options.level});
                return;
            }
            ...
            message = "GGER 网格 L" + renderable.level + "：" + renderable.cells.length + " 个可视单元";
            ...
            if (options.verticalEnabled) {
                message += "，GGER 高度层 " + heightModel.unitHeightMeters.toFixed(2) + "m，堆叠 " + heightModel.stackCount + " 层";
            }
        ...
        function handleClick(movement) {
            ...
            selection = {
                ...
                lat: lat,
                code2D: GGERGridCode.encode2D(bounds.centerLon, bounds.centerLat, level),
                code3D: GGERGridCode.encode3D(bounds.centerLon, bounds.centerLat, currentHeight, level),
                bounds: bounds,
            ...
            emitStatus("已选中 GGER 空域单元：" + selection.code2D, {
    ...
    function create(viewer, options) {
        if (!GGERGridCode || !GGERGridBounds) {
            throw new Error("GGERGridCode and GGERGridBounds are required");
        }
