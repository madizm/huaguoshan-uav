...
comment on view api.flight_path_plans is '路径规划方案/技术规划工作区列表，不是业务飞行计划；通过 RPC 创建、更新、计算或归档。';
...
comment on view api.flight_path_plan_points is '路径规划方案控制点。包含起点、终点和航路点。';
...
comment on view api.flight_path_plan_results is '路径规划结果列表；每条记录是一次平台计算输出，不是实际飞行事实。';
...
comment on function api.create_flight_path_plan(text, text, integer, double precision, text, timestamptz, jsonb, boolean, double precision, jsonb) is '创建路径规划方案/工作区并写入控制点，返回 plan_id；不是创建业务飞行计划。p_points 为控制点数组。';
comment on function api.update_flight_path_plan(bigint, text, text, integer, double precision, text, timestamptz, jsonb, boolean, double precision, jsonb) is '更新路径规划方案/工作区；传 null 的可选参数保持原值不变。p_points 非 null 时替换全部控制点。';
...
comment on function api.search_flight_path_results_by_bbox(double precision, double precision, double precision, double precision, timestamp, timestamp, integer, integer) is '按 WGS84 bbox 和可选时间范围分页查询成功的路径规划结果。参数：p_xmin、p_ymin、p_xmax、p_ymax、p_start_time、p_end_time、p_limit、p_offset。';
